namespace FastTFT128 {
    export namespace api {
        ...
        export function clear(color: number) { ensure(); ctx.fillStyle=css(color); ctx.fillRect(0,0,128,128) }
        export function fillRect(x:number,y:number,w:number,h:number,color:number) { ensure(); ctx.fillStyle=css(color); ctx.fillRect(x,y,w,h) }
        ...
        export function show() { createFramebuffer(); for(let y=0;y<128;y++) for(let x=0;x<128;x++) { ctx.fillStyle=css(fb[y*128+x]||0); ctx.fillRect(x,y,1,1) } }
    }
}
